...
public struct GenerateDynamicMemberFactory {
    private let subject: TokenSyntax

    private var members: [MemberBlockItemSyntax]
    private(set) var exclude: [Substring: StringLiteralExprSyntax]
...
extension GenerateDynamicMemberFactory {
    mutating func exclude(_ literal: StringLiteralExprSyntax) {
    ...
    consuming func factory(named name: TokenSyntax) -> DeclSyntax {
        """
        @frozen public
        struct \(name)
        {
...
extension GenerateDynamicMemberFactory: MemberMacro {
    public static func expansion(
        ...
        guard
        let decl: EnumDeclSyntax = .init(decl) else {
            context[.error, decl] = """
        ...
        for item: MemberBlockItemSyntax in decl.memberBlock.members {
            guard
            let cases: EnumCaseDeclSyntax = .init(item.decl) else {
                continue
            }
