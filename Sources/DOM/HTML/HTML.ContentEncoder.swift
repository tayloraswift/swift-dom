extension HTML {
    @frozen public struct ContentEncoder: StreamingEncoder {
...
extension HTML.ContentEncoder {
    @inlinable  static func += (self: inout Self, utf8: some Sequence<UInt8>) {
...
extension HTML.ContentEncoder: DOM.ContentEncoder {
    @usableFromInline typealias AttributeEncoder = HTML.AttributeEncoder

    /// Appends a *raw* UTF-8 code unit to the output stream.
    @inlinable public mutating func append(escaped codeunit: UInt8) {
...
extension HTML.ContentEncoder {
    ...
    @inlinable public mutating func append(unescaped codeunit: UInt8) {
    ...
    /// or ``subscript(_:_:content:) (_, _, (HTML.ContentEncoder) -> ())``.
    @inlinable public mutating func open(
        _ tag: HTML.ContainerElement,
    ...
    /// This is a low-level interface. Prefer encoding with ``subscript(_:content:)``
    /// or ``subscript(_:_:content:) (_, _, (HTML.ContentEncoder) -> ())``.
    @inlinable public mutating func close(_ tag: HTML.ContainerElement) {
...
extension HTML.ContentEncoder {
    @inlinable public subscript(
        ...
        content encode: (inout SVG.ContentEncoder) -> ()
    ) -> Void {
        mutating get {
            {
